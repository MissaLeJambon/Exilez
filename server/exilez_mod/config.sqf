...
// Global Settings
EZM_MaxZombies						= 150;				// How many Zombies overall?
EZM_ZombieSide						= WEST;             // zombie team side east, west and Civilian can be used.
EZM_MinSpawnDistance				= 40;               // Closest distance from any player to spawn a zombie.
EZM_MaxSpawnDistance				= 80;              // Max distance a zombie should spawn from a player.
EZM_MaxDistance						= 300;              // Max distance to players before delete.
EZM_MaxTime							= 15;               // Max time away from a player before delete.
EZM_MaxTimeDead						= 120;				// Max time for a dead Zombie to stay before delete. (Default 5 minutes)
...
EZM_TriggerGroupScaling				= 0.7;             	// 1 player = Groupsize, 2 player in trigger = Groupsize + (GroupSize * TriggerGroupScalling * number of player in the trigger) set at 0 to disable scaling

// Night settings
EZM_LightsOff						= true;             // Kill all the light on map except the player lights.
EZM_NightStartTime					= 19;               // Time at which it is night in hours
EZM_NightEndTime					= 7;                // Time at which it is no longer night in hours

// Explosive zombies
EZM_ExplosiveZombies				= true;				// randomly boobie trapped zombies exploding a few seconds after dying.
EZM_ExplosiveZombiesRatio			= 3;                // percentage of explosive zombies
EZM_ExplosiveZombieWarning			= "GRENADE !";		// Message that will display a few seconds before the explosion of a zombie.
EZM_ExplosionDelay					= 2;                // self-explanatory
EZM_ExplosiveType					= "mini_Grenade";	// "mini_Grenade" for small almost non-lethal explosion or "Grenade" Big and dangerous explosion.
EZM_ExplosiveRespect				= 20;             	// Bonus respect for Exploding zombies

// Killing zombies settings
EZM_EnableMoneyOnPlayer				= true;          	// Money goes directly on killer
EZM_EnableMoneyOnCorpse				= false;         	// Money stays on corpse for looting
EZM_ZombieMoney						= 20;               // Money per zombie kill
EZM_ZombieMaxMoney					= 0;               	// Max Money per zombie kill-random amount put on corpses
...
EZM_ZombieRespect					= 15;				// Respect per zombie kill

// Frag settings
EZM_RoadKillBonus					= 1;				// Bonus Respect if roadkill
EZM_MinDistance						= 10;				// Minimal distance for range bonus
EZM_CqbDistance						= 5;				// Minimal ditance for close quarter bonus
EZM_CqbBonus						= 5;				// Respect for close quarter bonus at 1 meter
...
EZM_ryanzombiesdamagetank                 = 0.001;	 	// Attack damage to tank *(% of car health per hit, 1 is 100%)
...
EZM_ryanzombiesinfectedchance             = 2;    		// Precent chances to be infected on hit (Default 0)
EZM_ryanzombiesinfectedrate               = 0.02;   	// Damage per minute when infected (+/- 30 minutes to live)
EZM_ryanzombiesinfectedsymptoms           = 0.7;    	// Symptomes showed when infected 0.9 = Normal 0.7 = Less 0.5 = None
EZM_ryanzombiesinfecteddeath              = 0.9;    	// 0.9 = Scream on death 0.7 = Silent death
EZM_ryanzombiesantivirusduration          = 400;    	// Antivirus duration *(5 minutes)
...
EZM_HarassingZedChance						= 80;		// Percent chance per player for a harassing Zed to spawn on them
...
EZM_HarassingConfig = [
/* 0 Groups Size  */         8,							// maximum number of zombies around a player
/* 1 Vest group */           EZM_Basic,				// Vest function defined in ZVest.sqf
/* 2 Loot group */           EZM_Level_1_Loot,				// Loot function defined in ZLoot.sqf
/* 3 Zombie group */         EZM_MediumMix				// Group function defined in ZClasses.sqf
];

// Hordes Config
EZM_UseHorde								= true;		// Use the horde spawner
EZM_HordeLoopTime							= 25;		// Spawn a Horde every X Minutes. (Default 30 mins)
EZM_HordeConfig = [
/* 0 Groups Size  */         35,						// maximum number of zombies around a player
/* 1 Vest group */           EZM_Basic,					// Vest function defined in ZVest.sqf
/* 2 Loot group */           EZM_Level_2_Loot,			// Loot function defined in ZLoot.sqf
/* 3 Zombie group */         EZM_HordeCall,				// Group function defined in ZClasses.sqf
/* 4 Horde density */        20							// Radius in which the zombies will spawn should be lower than Min Spawn Distance.
];
...
EZM_BlacklistExtendDistance					= 100;		// Extend the Traders Blacklist area by this much
